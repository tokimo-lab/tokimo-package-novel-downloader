...
pub struct TongrensheProvider;

pub fn provider() -> Box<dyn Provider> {
    Box::new(TongrensheProvider)
}

#[async_trait]
impl Provider for TongrensheProvider {
    fn name(&self) -> &str {
        "tongrenshe"
    }
    ...
    async fn search(
        ...
        let html = client
            .post_form(
                "https://tongrenshe.cc/e/search/index.php",
                &[
                    ("keyboard", keyword),
                    ("show", "title"),
                    ("classid", "0"),
        ...
        if let Ok(sel) = Selector::parse("div.books.m-cols div.bk") {
            for elem in doc.select(&sel).take(limit) {
                ...
                let author = select_text_in(&elem, "div.bk_right div.booknews")
                    .replace("作者：", "");
                let update_date =
                    select_text_in(&elem, "div.bk_right div.booknews label.date");
    ...
    async fn get_book_info(&self, client: &HttpClient, book_id: &str) -> Result<BookInfo> {
        let url = format!("https://tongrenshe.cc/tongren/{}.html", book_id);
        let html = client.get(&url).await?;
        let doc = Html::parse_document(&html);

        let mut info = BookInfo::default();
        info.book_name = select_text(&doc, "div.infos h1");
        info.author = select_text(&doc, "div.date span a");
    ...
    async fn get_chapter_content(
        ...
        let url = format!(
            "https://tongrenshe.cc/tongren/{}/{}.html",
            book_id, chapter_id
        );
        let html = client.get(&url).await?;
        let doc = Html::parse_document(&html);
