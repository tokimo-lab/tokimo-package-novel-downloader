...
impl SyosetuProvider {
    fn info_url(&self, book_id: &str, page: usize) -> String {
        if page > 1 {
            format!("https://ncode.syosetu.com/{}/?p={}", book_id, page)
        } else {
            format!("https://ncode.syosetu.com/{}/", book_id)
        }
    }
}

#[async_trait]
impl Provider for SyosetuProvider {
    fn name(&self) -> &str {
        "syosetu"
    }
    ...
    async fn get_book_info(&self, client: &HttpClient, book_id: &str) -> Result<BookInfo> {
        ...
        let first_html = client.get(&first_url).await?;
        ...
        if has_more_pages {
            ...
            loop {
                ...
                if let Ok(page_html) = client.get(&self.info_url(book_id, page)).await {
        ...
        // Parse volumes and chapters from all pages (non-async)
        let base = self.base_url().to_string();
        let volumes = Self::parse_all_volumes(&all_pages, &base);
